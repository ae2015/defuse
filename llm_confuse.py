...
'''
@File    :   llm_confuse.py
@Time    :   2024/10/13 23:38:36
@Author  :   Zhiyuan Peng 
@Version :   1.0
@Contact :   zpeng@scu.edu
@License :   (C)Copyright 2020-2021, zpeng/scu
@Desc    :   copy from llm_rag.py, commit id ca650cf, modify the code to generate confuse label by small llms.
'''
...
def find_false_assumptions_in_questions_v3(llm, documents, qr_schema, qr_path_in, qr_path_out, n=1):
    """
    based on find_false_assumptions_in_questions_v3, designed for llm_confuse.py
    """
    df_qr = utils.read_csv(qr_path_in, "Read the question-response table from CSV file")    
    print("Ask LLM to find a false assumption in each question, or say 'none'")
    rows_out = []
...
if __name__ == "__main__":
    ...
    cwd = os.getcwd()
    llm_models = ["gemma-2-27b-it", "gemma-2-9b-it", "Meta-Llama-3.1-8B-Instruct-Turbo", "Meta-Llama-3.1-70B-Instruct-Turbo", "Llama-3.2-3B-Instruct-Turbo", "Mistral-7B-Instruct-v0.3"]
    qrc_in_file = join(cwd, f"data/golden/unlabel/silver_99_117.csv")
    for llm_eval in llm_models:
        llm_start_time = utils.get_time()
        exp_folder = f"data/golden/unlabel/llmeval-{llm_eval}"
        os.makedirs(exp_folder, exist_ok = True)
        promptlib.read_prompts("prompts")
        print(f"\nAsk {llm_eval} to find the false assumption in each question\n")
        qrc_out_file = join(cwd, exp_folder, "qrc_2.csv")
        find_false_assumptions_in_questions_v3(llm_eval, all_docs, qrc_csv_schema, qrc_in_file, qrc_out_file, n=9)
        end_time = utils.get_time()
        print(f"{llm_eval} takes: {end_time - llm_start_time:.2f} seconds")
