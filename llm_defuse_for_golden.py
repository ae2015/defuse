...
@Desc    :   otc 14, 2024, copy from llm_defuse.py, commit id 5d2412f. Modify the code to for defuse gpt-3.5 of golden dataset.
'''
...
def check_if_response_defused_confusion_v4(llm, documents, qr_schema, qr_path_in, qr_path_out, mode="base", n=1, shot=2):
    """
    for defuse golden dataset
    """
    df_qr = utils.read_csv(qr_path_in, "Read the question-response table from CSV file")
    print("Ask LLM to check if its own response defused the confusion")
    rows_out = []
    for _, row in tqdm(df_qr.iterrows(), total = df_qr.shape[0]):
        ...
        if confusion == "no":
            ...
            if mode == "base":
                defusion, is_defused = promptlib.check_response_for_defusion_v3_base(llm, document, question, response, n, shot)
            else:
                defusion, is_defused = promptlib.check_response_for_defusion_v3(llm, document, question, response, n, shot)
        row_out = dict(row)
        row_out[qr_schema["defusion"]] = defusion
...
if __name__ == "__main__":
    ...
    news_num = 200 
    doc_prompt = "dt-z-1"
    modes = ["base", "exps", "consistency_3", "consistency_9"]
    ...
    for topic in topics:
        ...
        doc_file = join(data_folder, "docs_in.csv")
        doc_data = utils.read_csv(doc_file, f"Read {doc_file}")
        for _, row in doc_data.iterrows():
            all_docs[row["doc_id"]] = row["document"]
    cwd = os.getcwd()
    qrc_in_file = join(cwd, f"data/golden/unlabel/silver_99_117.csv")
    for mode in modes:
        start_time = utils.get_time()
        exp_folder = f"data/golden/unlabel/llm_defuse/llmeval-{llm_eval}"
        os.makedirs(exp_folder, exist_ok = True)
        promptlib.read_prompts("prompts")
        print(f"\nAsk {llm_eval} to judge whether the gpt-3.5 response defuse the q or not\n")
        qrc_out_file = join(cwd, exp_folder, f"qrc_2_{mode}.csv")
        if mode == "base":
            check_if_response_defused_confusion_v4(llm_eval, all_docs, qrc_csv_schema, qrc_in_file, qrc_out_file, mode=mode, n=1)
        elif mode == "exps":
            check_if_response_defused_confusion_v4(llm_eval, all_docs, qrc_csv_schema, qrc_in_file, qrc_out_file, mode=mode, n=1)
        elif mode == "consistency_3":
            check_if_response_defused_confusion_v4(llm_eval, all_docs, qrc_csv_schema, qrc_in_file, qrc_out_file, mode=mode, n=3)
        elif mode == "consistency_9":
            check_if_response_defused_confusion_v4(llm_eval, all_docs, qrc_csv_schema, qrc_in_file, qrc_out_file, mode=mode, n=9)
        end_time = utils.get_time()
        print(f"{llm_eval}, mode: {mode} takes: {end_time - start_time:.2f} seconds") 