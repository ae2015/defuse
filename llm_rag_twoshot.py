...
def generate_RAG_responses_2shot(llm, doc_schema, doc_path, qr_schema, qr_path):
    ...
    for _, row in tqdm(df_in.iterrows(), total = df_in.shape[0]):
        ...
        for q_id, is_conf, q in qs:
            response_q = promptlib.generate_response_2shot(llm, document, q)
            row_out = {
...
if __name__ == "__main__":
    ...
    for llm_r in llm_models:
        ...
        for topic in topics:
            ...
            exp_folder = f"data/exps/llmq-{llm_q}/docp-{doc_prompt}/{news_num}/{topic}/llmr-{llm_r}-twoshot"
            ...
            generate_RAG_responses_2shot(llm_r, doc_csv_schema, doc_paths["out"], qrc_csv_schema, qrc_paths[1])
