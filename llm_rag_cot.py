...
@Desc    :   otc 8, 2024, copy from datagen2.py commit id 9193865. Collection the responses of LLMs.
'''
...
def generate_RAG_responses_cot(llm, doc_schema, doc_path, qr_schema, qr_path):
    df_in = utils.read_csv(doc_path, "Read the document-and-questions table from CSV file")
    print("Generate RAG cot response for each question, both original and confusing")
    rows_out = []
    for _, row in tqdm(df_in.iterrows(), total = df_in.shape[0]):
        ...
        for q_id, is_conf, q in qs:
            response_q = promptlib.generate_response_cot(llm, document, q)
            row_out = {
...
if __name__ == "__main__":
    ...
    for llm_r in llm_models:
        ...
        for topic in topics:
            ...
            exp_folder = f"data/exps/llmq-{llm_q}/docp-{doc_prompt}/{news_num}/{topic}/llmr-{llm_r}-cot"
            ...
            generate_RAG_responses_cot(llm_r, doc_csv_schema, doc_paths["out"], qrc_csv_schema, qrc_paths[1])
