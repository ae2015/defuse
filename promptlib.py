...
def check_response_for_defusion_v3(llm, document, question, response, n, shot, prompt_key = "r-z-1"):
    prompt = []
    if rag_confusion_check[prompt_key]["system"]:
        prompt.append({
            "role" : "system",
...
def check_response_for_defusion_v3_scope(llm, document, question, response, n, shot, prompt_key = "r-z-1"):
    prompt = []
    if rag_confusion_check[prompt_key]["system"]:
        prompt.append({
            "role" : "system",
    ...
    example_document = examples_of_questions["zpeng-sport-5-5"]["document"]
    example_questions = examples_of_questions["zpeng-sport-5-5"]["facts"]["defuse_scope"]["question"]
    example_answers = examples_of_questions["zpeng-sport-5-5"]["facts"]["defuse_scope"]["responses"]
    example_reasonings = examples_of_questions["zpeng-sport-5-5"]["facts"]["defuse_scope"]["reasonings"]
    assert len(example_answers) == len(example_reasonings)
    ...
    prompt.append({
        "role" : "user",
        "content" : rag_confusion_check[prompt_key]["user_def_check_scope"]
    })
    prompt.append({
        "role" : "assistant",
        "content" : example_reasonings
